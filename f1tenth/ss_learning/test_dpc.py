...
def save_trajectory_csv(trajectory, path=None):
    if path is None:
        path = PLOT_DIR / "dpc_trajectory.csv"
    path = Path(path)
...
def reset_env_to_start(env, waypoints_track):
    start_idx = 0
    poses = np.array([[
        waypoints_track.raceline.xs[start_idx],
        waypoints_track.raceline.ys[start_idx],
        waypoints_track.raceline.yaws[start_idx],
    ]])
...
def main():
    ...
    primary_path = (
        ...
        else os.path.abspath(os.path.join(current_dir, "dpc_constrained_policy.pt"))
    )
    ...
    if save_data:
        ...
        out_csv = PLOT_DIR / f"dpc_trajectory_N{N}_dt{dt_ms}ms.csv"
