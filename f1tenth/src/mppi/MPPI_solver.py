...
class Const_MPPI_Solver(MPCSolver):
    """
    ...
    def _init_constraints_costs(self):
        ...
        # Get coefficients of the constraint Ax <= b
        def constraints_costs(x, con_coeffs):
            if len(constraints) == 0:
                return jnp.zeros((C, N), dtype=x.dtype)
        
            return jnp.stack([constraints[i](x, con_coeffs[i][0], con_coeffs[i][1]) for i in range(C)], axis=0)  # (C, N)
        
        return constraints_costs
    
    def _constraint_cost(self, x, con_coeffs):
        constraints = tuple(self.config.constraints)  # freeze (JIT-friendly)
        N = self.config.N
        C = self.config.n_constraints
        S = x.shape[0]
        ...
        if len(constraints) == 0:
            return jnp.zeros((S, C, N), dtype=x.dtype)

        violations = [
            jax.vmap(constraints[i], in_axes=(0, None, None))(
                x, con_coeffs[i][0], con_coeffs[i][1]
            )
            for i in range(C)
        ]

        return jnp.stack(violations, axis=1)
    ...
    def _init_control(self):
        ...
        if self.config.nu >= 2:
            a_opt = a_opt.at[:, 1].set(self.config.u_max[1])
        # a_cov: [N, nu, nu]
    ...
    def warm_start(self):
        """
        Warm start the control sequence from the solution at the previous timestep using shift operator.
        Args:
            a_opt (np.ndarray): control sequence of shape (N, nu) to warm start the optimal control input.
        Returns:
            None
        """
        a_opt_prev, a_cov_prev = self.control_params
        # Shift sequence left by one step and pad final control with zeros.
        a_opt = jnp.concatenate(
            [a_opt_prev[1:, :], jnp.expand_dims(self.nu_zeros, axis=0)],
            axis=0,
        )
        ...
        if self.config.adaptive_covariance:
            NotImplementedError("Adaptive covariance warm start not implemented yet")
        else:
            a_cov = (self.config.u_std**2) * jnp.tile(
                jnp.eye(jnp.array(self.config.nu)), (self.config.N, 1, 1)
            )
    ...
    def iteration_step(self, input_, env_state, ref_traj, con_coeffs, p, Q, R):
        ...
        c = self._constraint_cost(s, con_coeffs)  # [n_samples, C, N]
        ...
        c_candidates = self._constraint_cost(
            s_candidates, con_coeffs
        )  # [n_lambdas, C, N]
        ...
        r_penalized = r_modified[:, best_idx, :]  # [n_samples, N]
        ...
        if self.config.adaptive_covariance:
            ...
            # The following is suggested by AI
            # Weighted mean of perturbations at each time step: mu_da[t] in R^{nu}
            mu_da = jnp.sum(da * w_best[:, :, None], axis=0) # [N, nu]
            # Center the perturbations
            da_c = da - mu_da[None, :, :] # [S, N, nu]
            # Weighted covariance: Sigma[t] = Σ_s w[s,t] * da_c[s,t] da_c[s,t]^T
            Sigma = jnp.einsum(
                "sni,snj->nij", da_c, da_c * w_best[:, :, None]
            ) # [N, nu, nu]
            # Regularize to avoid rank loss / numerical issues
            Sigma = Sigma + 1e-5 * self.nu_eye[None, :, :] # [N, nu, nu]
    ...
    def solve(self, x0, ref_traj, const_coeffs, vis: bool = True, p=None, Q=None, R=None):
        """
        ...
        jax_con = [
            [jnp.array(coeffs[0]), jnp.array(coeffs[1])] for coeffs in const_coeffs
        ]
        ...
        if not self.config.scan:
            for _ in range(self.config.n_iterations):
                ...
                # print(-jnp.mean(r_sampled))
        else:
            def body(carry, _):
                carry, y = self.iteration_step(
                    carry, jax_x0, jax_ref, jax_con, self.p, self.config.Q, self.config.R
                )
                return carry, y

            (carry_final, ys) = jax.lax.scan(body, (a_opt, a_cov, rng), None,
                                            length=self.config.n_iterations)
            (a_opt, a_cov, rng) = carry_final
            (a_sampled, s_sampled, r_sampled) = ys

        self.control_params, self.samples = (
            (a_opt, a_cov),
            (a_sampled, s_sampled, jnp.sum(r_sampled, axis=1)),
        )
        # print(f"Optimal control input:\n {self.control_params[0].T}") 
        # print("-"*70) 
