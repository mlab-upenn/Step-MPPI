...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Warm-start DMPPI policy mean network from a trained DPC policy.",
    )
    parser.add_argument(
        "--dpc_policy_file",
        type=str,
        default=None,
        help="Path to DPC checkpoint used for warm start.",
    )
    return parser.parse_args()

def _resolve_dpc_warm_start_path(current_dir: str, custom_path: str | None) -> str:
    if custom_path is not None:
        return os.path.abspath(custom_path)
    primary_path = os.path.abspath(os.path.join(current_dir, "../ss_learning/dpc_constrained_policy.pt"))
    fallback_path = os.path.abspath(os.path.join(current_dir, "../saved_model/dpc_constrained_policy.pt"))
    return primary_path if os.path.exists(primary_path) else fallback_path

def warm_start_distribution_policy_from_dpc(policy: NeuralDistributionPolicy, dpc_ckpt_path: str, device: torch.device):
    ckpt = torch.load(dpc_ckpt_path, map_location=device, weights_only=True)
    dpc_state = ckpt.get("policy_state_dict", ckpt) if isinstance(ckpt, dict) else ckpt

    linear_indices = sorted(
        {
            int(k.split(".")[1])
            for k in dpc_state.keys()
            if k.startswith("net.") and k.endswith(".weight")
        }
    )
    if not linear_indices:
        raise ValueError(f"No DPC linear layers found in checkpoint: {dpc_ckpt_path}")
    last_linear_idx = linear_indices[-1]

    target_state = policy.state_dict()
    merged_state = dict(target_state)

    for t_key, t_val in target_state.items():
        s_key = None
        if t_key.startswith("backbone."):
            s_key = "net." + t_key[len("backbone."):]
        elif t_key.startswith("mean_head."):
            suffix = t_key.split(".", 1)[1]
            s_key = f"net.{last_linear_idx}.{suffix}"

        if s_key is not None and s_key in dpc_state and dpc_state[s_key].shape == t_val.shape:
            merged_state[t_key] = dpc_state[s_key].to(device=t_val.device, dtype=t_val.dtype)

    policy.load_state_dict(merged_state, strict=False)
    print(f"Warm-started DMPPI policy from DPC checkpoint: {dpc_ckpt_path}")
...
def main():
    ...
    base_save_path = Path(tp.get("save_path", "dmppi_policy.pt"))
    ...
    if args.warm_start_dpc and args.retrain:
        print("Skipping DPC warm start because --retrain loads a DMPPI checkpoint.")
    elif args.warm_start_dpc:
        dpc_ckpt_path = _resolve_dpc_warm_start_path(current_dir, args.dpc_policy_file)
        warm_start_distribution_policy_from_dpc(policy, dpc_ckpt_path, device)
    ...
    if args.retrain:
        primary_path = os.path.abspath(os.path.join(current_dir, "dmppi_constrained_policy_N100_dt50ms.pt"))
